...
void HcalAna::Analysis() { 
	...
	hfolder=((ofile+"/"+ifile+"/SLHC_"+string(hfName)).c_str());
	...
	for ( int i=0; i< totalN ; i++ ) //particular event 
	{
		...
		for(int k=0;k<nGen; k++)//DUMBASS THIS DOES REALality nGen
		{
			...
			if (abs(bid[k])==type&& abs(mom[k])==24&&gP4.Pt()>mucuts[0]&&fabs(gP4.Eta())<mucuts[2])//sees if it comes from a w boson
			{
			...
			if (abs(bid[k])==type&& abs(mom[k])!=24&&gP4.Pt()>mucuts[0]&&eta( bPx[k], bPy[k], bPz[k] )<mucuts[2])// sees if it is a muon form a jet
			{
			...
			if (abs(bid[k])==type&&gP4.Pt()>mucuts[0]&&eta( bPx[k], bPy[k], bPz[k] )<mucuts[2])
			{
				//THIS IS TOTAL
				//
				...
				if (abs(bid[k])==type&&gP4.Pt()>mucuts[0]&&gP4.Eta()<mucuts[2])
				{
