...
HcalAna::HcalAna( string datacardfile ) {
	...
	if(sparameter1[0]==4)ofile=ofile+"comp";
   ...
   snparameterh3[0]=snparameterh2[0]=snparameterh1[0];
	...
	////////////
	/*
	cout<<"nparameter1[0] "<<nparameter1[0]<<endl;
	cout<<"nparameter1[1] "<<nparameter1[1]<<endl;
	cout<<"nparameter2[0] "<<nparameter2[0]<<endl;
	cout<<"nparameter2[1] "<<nparameter2[1]<<endl;
	cout<<"nparameter3[0] "<<nparameter3[0]<<endl;
	cout<<"nparameter3[1] "<<nparameter3[1]<<endl;
	*///you can delete this
...
/*
		...
		for ( int k=0; k< nMu; k++ ) {//shit should be reco muons
			...
			TLorentzVector mP4( lepPx[k], lepPy[k], lepPz[k], lepE[k] ) ;

			//cout<<" momentum="<<mP4.Pt()<<endl;
			if(mP4.Pt()>mucuts[0]&& eta( lepPx[k], lepPy[k], lepPz[k] )<mucuts[2])
			{h_lepPt->Fill( mP4.Pt() ) ;
				...
				//cout<<"eta="<<eta( lepPx[k], lepPy[k], lepPz[k] )<<" so the momentum is="<<mP4.Pt()<<endl;
				count++;
				//cout<<"so our fake eta is accepted"<<eta( lepPx[k], lepPy[k], lepPz[k] )<<" the pt is"<<mP4.Pt()<<endl;
				if( parameter1[0]==4)
				{
				...
				if( parameter2[0]==4)
				{
					mesht1+=(iso(parameter2[1],k,parameter1[2]));
				}
				if( parameter3[0]==4)
				{
					mesht1+=(iso(parameter3[1],k,parameter1[2]));
				}
				///////
				if( nparameter1[0]==4)
				{
				...
				if( nparameter2[0]==4)
				{
					mesht1-=(iso(nparameter2[1],k,nparameter1[2]));
				}
				if( nparameter3[0]==4)
				{
					mesht1-=(iso(nparameter3[1],k,nparameter1[2]));
				}       
				...
				if( sparameter2[0]==4)
				{
					mesht2+=(iso(sparameter2[1],k,sparameter1[2]));
				}
				if( sparameter3[0]==4)
				{
					mesht2+=(iso(sparameter3[1],k,sparameter1[2]));
				}
				//start of hits recon
				////////////
				if( snparameter1[0]==4)
				...
				if( snparameter2[0]==4)
				{
					mesht2-=(iso(snparameter2[1],k,snparameter1[2]));
				}
				if( snparameter3[0]==4)
				{
					mesht2-=(iso(snparameter3[1],k,snparameter1[2]));
				}
				/////////     

				if( parameterh1[0]==4)
				{
					mesht1=(isoh(parameterh1[1],k,parameterh1[2]));
				}
				if( parameterh2[0]==4)
				{
				...
				if( parameterh3[0]==4)
				{
					mesht1+=(iso(parameterh3[1],k,parameterh1[2]));
				}
				/////////////
				if( nparameterh1[0]==4)
				{
					mesht1-=(isoh(nparameterh1[1],k,nparameterh1[2]));
				}
				if( nparameterh2[0]==4)
				{
					mesht1-=(iso(nparameterh2[1],k,nparameterh1[2]));
				}
				if( nparameterh3[0]==4)
				{
					mesht1-=(iso(nparameterh3[1],k,nparameterh1[2]));
				}        
				...
				{
					mesht2-=(iso(snparameterh1[1],k,snparameterh1[2]));
				}
				if( snparameterh2[0]==4)
				{
					mesht2-=(iso(snparameterh2[1],k,snparameterh1[2]));
				}
				if( snparameterh3[0]==4)
				{
					mesht2+=(iso(snparameterh3[1],k,snparameterh1[2]));
				}
			...
			// cout<<"so our fake eta is not necassarly"<<eta( lepPx[k], lepPy[k], lepPz[k] )<<endl;
...
void HcalAna::Draw_H1( vector <TH1D*> h1 , string plotname, vector <TH1D*> h2 /* = NULL */, string splotname /*""*/ ) {
	...
	TString plotname3 = ofile+"/"+ifile+"/compare/"+comp1+sub+"_"+comp2+sub+"."+plotType ;
	c3->Print( plotname3 );
	/////////
	string helpplease = ofile+"/"+ifile+"/compare/"+comp1+sub+"_t"+comp2+sub+".root";
	...
	c4->cd();
	ratio->Draw();
	TString plotname4 = ofile+"/"+ifile+"/compare/ratio"+comp1+sub+"_"+comp2+sub+"."+plotType ;
	c4->Print( plotname4 );
	delete c4;
	/////////////
...
void HcalAna::Draw_H2( vector <TH1D*> h1 , string plotname, vector <TH1D*> h2 /* = NULL */, string splotname /*""*/ ) {
	...
	TString plotname3 = ofile+"/"+ifile+"/compare/"+comp1+sub+"h_"+comp2+sub+"."+plotType ;
	c3->Print( plotname3 );
	string helpplease = ofile+"/"+ifile+"/compare/"+comp1+sub+"h_t"+comp2+sub+".root";
	TFile * gt = new TFile(helpplease.c_str(), "RECREATE");
	gt->cd();
	mg->Write();
