...
void HcalAna::Analysis() { 
	...
	TString Path_fName = hfolder + "/" + hfName + ".root" ;//probably dont need to worry about 
	...
	for(int i=0;i<6;i++)
	{
		...
		meshh1.push_back(   new TH1D(meshh1_name, "so this is whatever you chose to add together deposit hits", 50, 0, 50));
	...
	for ( int i=0; i< totalN ; i++ ) //particular event 
	{
		...
		for(int k=0;k<nGen; k++)//DUMBASS THIS DOES REALality nGen
		{
			...
			if (abs(bid[k])==13&& abs(mom[k])==24&&gP4.Pt()>mucuts[0]&&fabs(gP4.Eta())<mucuts[2])//sees if it comes from a w boson
			{
				...
				massTableofDoom(1, k, mesht1, mesht2, meshht1, meshht2);
				////new
			}


			if (abs(bid[k])==13&& abs(mom[k])!=24&&gP4.Pt()>mucuts[0]&&eta( bPx[k], bPy[k], bPz[k] )<mucuts[2])// sees if it is a muon form a jet
			{
				...
				evngnz++;
				massTableofDoom(2, k, mesht1, mesht2, meshht1, meshht2);
				
				
			}

			if (abs(bid[k])==13&&gP4.Pt()>mucuts[0]&&eta( bPx[k], bPy[k], bPz[k] )<mucuts[2])
			{
				//THIS IS TOTAL
				//
				massTableofDoom(3, k, mesht1, mesht2, meshht1, meshht2);
				...
				if (abs(bid[k])==13&&gP4.Pt()>mucuts[0]&&gP4.Eta()<mucuts[2])
				{
					...
					if((parameterh1[0]==1&&abs(mom[k])==24)||(parameterh1[0]==2&&abs(mom[k])!=24)||(parameterh1[0]==3)||(parameterh1[0]==4))
					{
						...
						if(parameterh1[3]==2)
						{
							meshh1.at(hisn)->Fill(meshht1/muE[k]);
						}
						if(parameterh1[3]==1)
						{
					...
					if((sparameterh1[0]==1&&abs(mom[k])==24)||(sparameterh1[0]==2&&abs(mom[k])!=24)||(sparameterh1[0]==3)||(sparameterh1[0]==4))
					{
						if(sparameterh1[3]==2)
						{
							meshh2.at(hisn)->Fill(meshht2/muE[k]);
						}
						if(sparameterh1[3]==1)
						{
		...
		for ( int k=0; k< nMu; k++ ) {//shit should be reco muons
			...
			{h_muPt->Fill( mP4.Pt() ) ;
				...
				{
					mesh2.at(hisn)->Fill(mesht2/muE[k]);
				}
...
void HcalAna::Draw_H1( vector <TH1D*> h1 , string plotname, vector <TH1D*> h2 /* = NULL */, string splotname /*""*/ ) {
	...
	gSystem->mkdir("compare");

	TString plotname3 = "compare/"+comp1+"_"+comp2+"."+plotType ;
	c3->Print( plotname3 );
	/////////
	string helpplease = "compare/"+comp1+"_t"+comp2+".root";
	...
	c4->cd();
	ratio->Draw();
	TString plotname4 = "compare/ratio"+comp1+"_"+comp2+"."+plotType ;
	c4->Print( plotname4 );
	delete c4;
	/////////////
...
void HcalAna::Draw_H2( vector <TH1D*> h1 , string plotname, vector <TH1D*> h2 /* = NULL */, string splotname /*""*/ ) {
	...
	for(int k=0;k<2;k++)
	{
		...
		hfolder="SLHC_"+comp.at(k)+"/";
		cout<<"does  this make sense2?"<<parameterh1[1]<<endl;
		TCanvas* c1 = new TCanvas("c1","", 800, 700);
		c1->SetFillColor(10);
		c1->SetFillColor(10);
		//c1->SetLogy();
		...
		cout<<"does  this make sense?"<<parameterh1[1]<<endl;
	...
	TString plotname3 = "compare/"+comp1+"h_"+comp2+"."+plotType ;
	c3->Print( plotname3 );
	string helpplease = "compare/"+comp1+"h_t"+comp2+".root";
	TFile * gt = new TFile(helpplease.c_str(), "RECREATE");
	gt->cd();
	mg->Write();
	...
	c4->cd();
	ratio->Draw();
	TString plotname4 = "compare/ratioh"+comp1+"_"+comp2+"."+plotType ;
	c4->Print( plotname4 );
	delete c4;
	/////////////
}
...
{/* 
	g_isolw1->Fill(giso(par,k,0));
	g_isolw12->Fill(giso(par,k,0)+giso(par,k,1));
	g_isolw123->Fill(giso(par,k,0)+giso(par,k,1)+giso(par,k,2));


	g_isolwh1->Fill(gisoh(par,k,0));
	g_isolwh12->Fill(gisoh(par,k,0)+gisoh(par,k,1));
	g_isolwh123->Fill(gisoh(par,k,0)+gisoh(par,k,1)+gisoh(par,k,2));*/
	//cout<<"testing please"<<endl;
	            if( parameter1[0]==par)
	            {
	                mesht1=(giso(parameter1[1],k,parameter1[2]));
	            }
				if( parameter2[0]==par)
				{
					mesht1+=(giso(parameter2[1], k,parameter2[2]));
				}
				if( parameter3[0]==par)
				{
					mesht1+=(giso(parameter3[1],k,parameter3[2]));
				}
				//subtracting
				if( nparameter1[0]==par)
				{
					mesht1-=(giso(nparameter1[1],k,nparameter1[2]));


				}
				if( nparameter2[0]==par)
				{
					mesht1-=(giso(nparameter2[1], k,nparameter2[2]));
				}
				if( nparameter3[0]==par)
				{
					mesht1-=(giso(nparameter3[1],k,nparameter3[2]));
				...
				{
					mesht2=(giso(sparameter1[1],k,sparameter1[2]));
				}
				if( sparameter2[0]==par)
				{
					mesht2+=(giso(sparameter2[1],k,sparameter2[2]));
				}
				if( sparameter3[0]==par)
				{
					mesht2+=(giso(sparameter3[1],k,sparameter3[2]));
				}
				//////////subtracting
				if( snparameter1[0]==par)
				{
					mesht2-=(giso(snparameter1[1],k,snparameter1[2]));
				}
				if( snparameter2[0]==par)
				{
					mesht2-=(giso(snparameter2[1],k,snparameter2[2]));
				}
				if( snparameter3[0]==par)
				{
					mesht2-=(giso(snparameter3[1],k,snparameter3[2]));
				}
				//////
				//okay hits w boson
				if( parameterh1[0]==par)
				{
					meshht1=(gisoh(parameterh1[1],k,parameterh1[2]));
				}
				if( parameterh2[0]==par)
				{
					meshht1+=(gisoh(parameterh2[1],k,parameterh2[2]));
				}
				if( parameterh3[0]==par)
				{
					meshht1+=(gisoh(parameterh3[1],k,parameterh3[2]));
				}
				//subtracting
				...
				{
					meshht1-=(gisoh(nparameterh1[1],k,nparameterh1[2]));

				}
				if( nparameterh2[0]==par)
				{
					meshht1-=(gisoh(nparameterh2[1],k,nparameterh2[2]));
				}
				if( nparameterh3[0]==par)
				{
					meshht1-=(gisoh(nparameterh3[1],k,nparameterh3[2]));

				}
				///////////
				if( sparameterh1[0]==par)
				{
					meshht2=(gisoh(sparameterh1[1],k,sparameterh1[2]));
				}
				if( sparameterh2[0]==par)
				{
					meshht2+=(gisoh(sparameterh2[1],k,sparameterh2[2]));
				}
				if( sparameterh3[0]==par)
				{
					meshht2+=(gisoh(sparameterh3[1],k,sparameterh3[2]));
				}
				////////////////subtracting
				if( snparameterh1[0]==par)
				{
					meshht2-=(gisoh(snparameterh1[1],k,snparameterh1[2]));
				}
				if( snparameterh2[0]==par)
				{
					meshht2-=(gisoh(snparameterh2[1],k,snparameterh2[2]));
				}
				if( snparameterh3[0]==par)
				{
					meshht2-=(gisoh(snparameterh3[1],k,snparameterh3[2]));
				}
}
